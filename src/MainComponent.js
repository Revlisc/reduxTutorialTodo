...
import { addTodo, clearAllTasks, createTask, deleteAllTasks, deleteThis, toggleToDo } from './redux/ActionCreators'
// Task:  Import functions from ActionCreators
//import { ToDo } from './redux/todo.js';
...
const mapDispatchToProps = (dispatch) => ({
  ...
  deleteThis: (idx) => dispatch(deleteThis(idx))
});
        ...
        <ul>
          {/* Task: create a map that displays the list item. don't forget the unique key. we will be using the index of the array*/
            ToDo.todo.map((item, idx) => {
              ...
              <button onClick={() => this.props.deleteThis(idx)}>Delete</button>
